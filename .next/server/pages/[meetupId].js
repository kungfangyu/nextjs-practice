...
async function getStaticPaths() {
  const client = await external_mongodb_["MongoClient"].connect('mongodb+srv://Fane:ATFDyq3JQ0bkTuiN@cluster0.9rbz6.mongodb.net/meetups?retryWrites=true&w=majority');
  const db = client.db();
  const meetupsCollection = db.collection('meetups');
  const meetups = await meetupsCollection.find({}, {
    _id: 1
  }).toArray(); //find => 這裡的第一個空物件代表要找的數據沒有過濾條件，就是全部的對象都要,第二個參數代表哪些數需要被提取出來
